...
userController.updateUser = async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            if (status) {
                res.status(200).send({ message: "Update user successed!" })
            }
        } else {
            const error = new Error('User not found!');
            error.statusCode = httpStatus.NOT_FOUND;
...
userController.changePassword = async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            const status = await User.updateUser(id, { password: newPassword })
            if (status) {
...
userController.deleteOne = async (req, res, next) => {
    ...
    try {
        ...
        if (!data) {
            ...
            res.status(200).send({ message: `Delete successed user id ${id}` })
        }
    } catch (e) {
        if (!e.statusCode) {
            e.statusCode = httpStatus.INTERNAL_SERVER_ERROR;
