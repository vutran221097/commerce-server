...
productController.search = async (req, res, next) => {
    ...
    try {
        ...
        const skip = (page - 1) * pageSize;
        const totalPage = Math.ceil(totalProduct / pageSize);
        const data = await Product.getAllProducts({ name: { $regex: new RegExp(search), $options: 'i' } }, initSort, skip, pageSize)
...
productController.findByCategory = async (req, res, next) => {
    try {
        ...
        const totalProducts = await Product.getAllProducts({ category: type })
        ...
        const skip = (page - 1) * pageSize;
        const totalPage = Math.ceil(totalProduct / pageSize);
        const data = await Product.getAllProducts({ category: type }, sort, skip, pageSize)
...
productController.updateOne = async (req, res, next) => {
    try {
        ...
        if (product) {
            res.status(200).send({ message: "Update product successed!" })
        } else {
            const error = new Error('Product not found!');
            error.statusCode = httpStatus.NOT_FOUND;
            throw error;
...
productController.deleteOne = async (req, res, next) => {
    ...
    try {
        ...
        if (prod) {
            ...
            if (data) {
                res.status(200).send({ message: `Delete success product id ${id}` })
            }
        }
